#!/usr/bin/env python3
"""
Send a WAV audio request to Voxtral via JSON ingress and wait for one result.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Send WAV audio to Voxtral and wait for inference result."
    )
    parser.add_argument("--wav", required=True, help="Path to WAV file.")
    parser.add_argument("--prompt", default="", help="Optional text instruction.")
    parser.add_argument("--target", default=DEFAULT_TARGET, help="Target package name.")
    parser.add_argument("--source-id", default="mic/upload", help="Ingress source_id.")
    parser.add_argument(
        "--pipe",
    ...
    parser.add_argument(
        ...
        type=float,
        default=8.0,
        help="Trim audio to at most this duration before sending.",
    )
    parser.add_argument(
        "--aimod",
...
def read_wav_bytes(
    ...
    with wave.open(str(wav_path), "rb") as wav:
        ...
        if max_seconds > 0:
            max_frames = min(total_frames, int(sample_rate * max_seconds))

        raw = wav.readframes(max_frames)
        return raw, sample_rate, channels, sample_width
...
def main() -> int:
    ...
    try:
        wav_bytes, sample_rate, channels, sample_width = read_wav_bytes(
            wav_path, args.max_seconds
        )
    except Exception as exc:
        print(f"Failed to read WAV file '{wav_path}': {exc}", file=sys.stderr)
        return 2

    if not wav_bytes:
        print("WAV payload is empty after trimming.", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(
            f"Failed to send audio input to pipe '{args.pipe}': {exc}", file=sys.stderr
        )
        print(
